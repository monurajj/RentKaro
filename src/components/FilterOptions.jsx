...
// ... (PriceFilter, OccupancyFilter, FacilitiesFilter, SortingOptions, and StateFilter components remain the same)
...
const SortingOptions = ({ sortOrder, setSortOrder }) => (
  <FilterOption icon={sortOrder === 'asc' ? FaSortAmountUp : FaSortAmountDown} label="Sort by Price">
    <div className="flex justify-between mt-2">
      <button
        className={`px-3 py-1 rounded ${sortOrder === 'asc' ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700'}`}
        onClick={() => setSortOrder('asc')}
      >
        Low to High
      </button>
      <button
        className={`px-3 py-1 rounded ${sortOrder === 'desc' ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700'}`}
